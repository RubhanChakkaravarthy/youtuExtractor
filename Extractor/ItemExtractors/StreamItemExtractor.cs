...
{
	internal class StreamItemExtractor : IItemExtractor<StreamItem>
	{
		private readonly JsonObject _streamItemObject;

		public StreamItemExtractor(JsonObject streamItemObject)
		{
		...
		{
			if (_streamItemObject.TryGetOneOf(new [] { "title", "headline" }, out JsonObject nameObj))
				return ParsingHelpers.GetText(nameObj);
			return default;
		}
		...
		{
			if (_streamItemObject.TryGetObject("descriptionSnippet", out var descriptionObj))
				return ParsingHelpers.GetText(descriptionObj);
			return default;
		}
		...
		public ChannelItem GetUploader()
		{
			...
			if (uploaderData != null)
			{
				...
				{
					channel.IsVerified = ParsingHelpers.IsChannelVerified(_streamItemObject.GetArray("ownerBadges"));
				}
